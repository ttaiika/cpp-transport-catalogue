...
/**
 * Парсит расстояния до других остановок из строчки с остановкой
 */
std::unordered_map<std::string, double> ParseDistances(std::string_view str) {
    std::unordered_map<std::string, double> distances;
    std::regex regex(R"((\d+)m to ([\w\s]+))");
    std::string str_copy = std::string(str);
    std::smatch match;

    while (std::regex_search(str_copy, match, regex)) {
        double distance = std::stod(match[1].str());
        std::string stop_name = match[2].str();
        distances[stop_name] = distance;
        str_copy = match.suffix().str();
    }
...
void Reader::ApplyCommands(Catalogue& catalogue) const {
    ...
    for (const auto& command : commands_) {
        if (command.command == "Stop") {
            ...
            // Извлекаем расстояния до других остановок
            ...
            temp_stops[stop_name].distances = distances;
        }
    ...
    for (const auto& [stop_a_name, map_distances] : temp_stops) {
        for (const auto& [stop_b_name, dist] : map_distances.distances) {
            auto a = catalogue.FindStop(stop_a_name);
            auto b = catalogue.FindStop(stop_b_name);
            auto stops = std::make_pair(a, b);

            catalogue.SetDistance(stops, dist);
        }
    }
